...
The CLI will run every SQL file in [`supabase/migrations`](../supabase/migrations) (including the new account-type migration `0003_add_account_type_and_carpenter_tables.sql` and the avatar bucket provisioning script `0005_provision_avatar_bucket.sql`) against the linked project.
